...
class TemplateService
{
    ...
    {
        if ($request->expectsJson()) {
            $row = Template::where('organization_id', $this->organizationId)
                ->where('uuid', $uuid)
        ...
        $data['template'] = Template::where('organization_id', $this->organizationId)
            ->where('uuid', $uuid)
    ...
    public function createTemplate(Request $request)
    {
        if ($request->isMethod('get')){
            ...
            $metadata = $data['settings']?->metadata ? json_decode($data['settings']->metadata, true) : [];
            $data['templateReadiness'] = app(WhatsappTemplateReadinessService::class)->buildForMetadata($metadata);
    ...
    protected function abortIfDemo($type)
    {
        if (app()->environment('demo') && $this->organizationId == 1) {
            if($type == 'delete'){
                return response()->json([
                    'success' => false,
                    'message' => __('You are not allowed to do this in this demo account. To test template features please create your own account.')
                ]);
            } else {
                ...
                $responseObject->data->error->message = __('You are not allowed to do this in this demo account. To test template features please create your own account.');
