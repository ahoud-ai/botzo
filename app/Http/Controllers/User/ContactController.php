...
class ContactController extends BaseController
{
    ...
    public function index(Request $request, $uuid = null){
        ...
        $this->checkPermission('contacts.view_all', $organizationId);
    ...
    {
        $organizationId = $this->getCurrentOrganizationId();
        $this->checkPermission('contacts.export', $organizationId);
    ...
    public function import(Request $request) {
        $organizationId = $this->getCurrentOrganizationId();
        $this->checkPermission('contacts.import', $organizationId);
    ...
    public function store(StoreContact $request){
        $organizationId = $this->getCurrentOrganizationId();
        $this->checkPermission('contacts.create', $organizationId);
    ...
    {
        $organizationId = $this->getCurrentOrganizationId();
        $this->checkPermission('contacts.edit', $organizationId);
    ...
    {
        $organizationId = $this->getCurrentOrganizationId();
        $this->checkPermission('contacts.delete', $organizationId);
    ...
    public function assignGroup(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'uuids.*' => [
                'required',
                'string',
                Rule::exists('contacts', 'uuid')->where(function ($query) use ($organizationId) {
                    return $query->where('organization_id', $organizationId)->whereNull('deleted_at');
                }),
            ],
        ], [
            'group_uuid.required' => __('Please select a group.'),
            'uuids.required' => __('Please select at least one contact.'),
            'uuids.min' => __('Please select at least one contact.'),
        ]);
        ...
        $result = $this->contactService()->assignContactsToGroup(
            $request->input('uuids', []),
            $request->input('group_uuid')
        );
        ...
        return Redirect::back()->with(
            ...
            [
                'type' => 'success',
                'message' => __('Selected contacts were added to the group successfully.'),
                'meta' => $result,
            ]
        );
    }
