<template>
    <div v-if="props.displayTopBar === true" class="ui-mobile-topbar creative-mobile-topbar sticky top-0 ui-layer-content w-full bg-white dark:bg-[#0a0f17] border-b dark:border-white/10 px-4 py-4 flex items-center justify-between md:hidden">
        <div>
        ...
        <div class="flex items-center gap-2">
            <Link v-if="props.displayCreateBtn" :href="createHref">
    ...
    <aside
        class="ui-mobile-drawer ui-layer-drawer creative-mobile-drawer transform top-0 w-full bg-white dark:bg-[#0a0f17] fixed h-full overflow-auto ease-in-out transition-all duration-300"
        :class="[drawerPlacementClass, isSidebarOpen ? 'translate-x-0' : drawerClosedClass]">
        <aside class="flex flex-col h-full w-full bg-white dark:bg-[#0a0f17]">
    ...
    <ProfileModal :user="props.user" :organization="props.organization" :isOpen="isProfileOpen" role="user" :languages="languages" @close="closeModal()"/>
</template>
<script setup>
