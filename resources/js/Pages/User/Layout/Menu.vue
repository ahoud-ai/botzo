<template>
    <div v-if="!getValueByKey('logo') || getValueByKey('logo') === null" class="ui-menu creative-menu flex items-center justify-between px-5 pt-5 h-20 mb-4">
        <h2 v-if="!menuIconsOnly" class="ms-2 text-2xl">{{ getValueByKey('company_name') }}</h2>
    ...
    <div v-else class="ui-menu creative-menu flex items-center justify-between px-5 pt-5 h-20 mb-1">
        <Link href="/dashboard">
    ...
    <div class="ui-menu flex-grow space-y-3 px-2 overflow-y-auto">
        <div class="flex-1">
            <ul class="pt-2 space-y-1 text-sm mb-2">
                <li class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/dashboard') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/dashboard" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canViewChats" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/chats') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/chats" class="flex items-center justify-between p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canViewContacts" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/contact') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/contacts" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canViewCampaigns" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/campaign') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/campaigns" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canViewTemplates" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/template') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/templates" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canViewAutomations" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/automation') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/automation/basic" class="flex items-center p-2 gap-x-3 rounded-md">
            ...
            <ul class="pb-4 space-y-1 text-sm mt-2">
                <li v-if="canManageSettings" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/settings') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/settings" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canViewBilling" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/billing') || $page.url.startsWith('/subscription') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/billing" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate" :class="$page.url.startsWith('/support') ? 'bg-slate-50 text-black' : ''">
                    <Link rel="noopener noreferrer" href="/support" class="flex items-center p-2 gap-x-3 rounded-md">
                ...
                <li v-if="canAccessDeveloperTools" class="hover:bg-slate-50 hover:text-black rounded-md px-2 truncate">
                    <Link rel="noopener noreferrer" href="/developer-tools/access-tokens" class="flex items-center p-2 gap-x-3 rounded-md">
    ...
    <div v-if="menuIconsOnly === false" @click="switchTeams()" class="creative-team-switch border-2 border-primary text-sm rounded-md mb-1 m-3 py-2 px-4 flex items-center justify-between cursor-pointer">
        <div class="flex min-w-0 flex-col">
            <div class="flex gap-x-1">
                <span>{{ $t('Workspace') }}:</span>
                <span class="max-w-[10rem] truncate md:max-w-[11rem]">{{ props.organization.name }}</span>
            </div>
            <span v-if="currentWorkspaceContext" class="text-[11px] text-slate-600">{{ currentWorkspaceContext }}</span>
        </div>
        <span>
    ...
    <div class="creative-profile-card flex items-center m-3 p-2 rounded-xl h-14 py-1 md:py-1 mt-2 gap-x-4 bg-slate-50" :class="!menuIconsOnly ? 'justify-between' : 'justify-center'">
        <div v-if="!menuIconsOnly" class="flex gap-x-2">
            <div class="rounded-xl p-1 bg-slate-200">
                <img v-if="user.avatar" class="rounded-full w-9 h-9" :src="'/media/' + user.avatar" alt="">
            ...
            <div>
                <h2 class="max-w-[10rem] truncate text-[15px] capitalize md:max-w-[11rem]"  :class="menuIconsOnly ? 'hidden' : ''">{{ user.first_name + ' ' + user.last_name }}</h2>
                <span class="flex items-center gap-x-1" :class="menuIconsOnly ? 'hidden' : ''">
                    <span @click="openModal" class="text-sm hover:underline dark:text-gray-400 cursor-pointer">{{ $t('View profile') }}</span>
                </span>
            </div>
        </div>
        <Link href="/logout" method="post" as="button" type="button" class="hover-ui-bg-soft hover:rounded-full w-[fit-content] p-2">
