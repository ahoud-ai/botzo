    ...
    <AppLayout>
        <div>
            ...
            <p class="mb-6 flex items-center text-sm leading-6 text-gray-600">
                ...
                <span class="ms-1 mt-1">{{ $t('Configure email accounts') }}</span>
            </p>
        </div>
            ...
            <div class="space-y-12">
                <div class="pb-12">
                    <div class="pb-10">
                        ...
                        <div v-else-if="form.mail_config.driver === 'resend'" class="grid gap-6 grid-cols-2 pb-6 md:w-2/3">
                            <FormInput v-model="form.mail_config.resend_api_key" :error="form.errors['mail_config.resend_api_key']" :name="$t('Resend API Key')" :type="'password'" :class="'col-span-2'"/>
                        </div>
                            ...
                            <div class="relative flex gap-x-3 mt-4 col-span-2">
                                <div class="flex items-center">
                                    <label @click="toggleSmtp()" for="myCheckbox" class="cursor-pointer">
                                        <div class="w-4 h-4 border border-gray-400 rounded-md flex items-center justify-center" :class="form.smtp_email_active ? 'bg-[color:var(--ui-text)]' : ''">
                                    ...
                                    <span @click="toggleSmtp()" class="ms-2 text-[14px] cursor-pointer">{{ $t('Activate email') }}</span>
                                </div>
                            </div>
                            <div class="relative flex gap-x-3 mt-4 col-span-2">
                                <div class="flex items-center">
                                    <label @click="toggleVerification()" for="myCheckbox" class="cursor-pointer">
                                        <div class="w-4 h-4 border border-gray-400 rounded-md flex items-center justify-center" :class="{ 'bg-[color:var(--ui-text)]': form.verify_email }">
                                    ...
                                    <span @click="toggleVerification()" class="ms-2 text-[14px] cursor-pointer">{{ $t('Require email verification for new accounts') }}</span>
                                </div>
                            </div>
                            <!--<div class="mt-6">
                                <button type="submit" :class="['inline-flex justify-center rounded-md border border-transparent bg-primary px-4 py-2 text-sm text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2', { 'opacity-50': isLoading }]">
