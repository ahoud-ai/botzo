<script setup>
    ...
    const setDefaultAction = (code) => {
        form.post('/admin/languages/' + code + '/default');
    }
    ...
    <Table :rows="rows">
        ...
        <TableBody>
            <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                ...
                <TableBodyRowItem>
                    <span v-if="props.defaultLanguage == item.code" class="bg-slate-50 p-1 rounded-md text-[12px]">{{ $t('Default Language') }}</span>
                </TableBodyRowItem>
                <TableBodyRowItem :position="'last'">
                    ...
                    <template #items>
                        <DropdownItemGroup>
                            <DropdownItem v-if="props.defaultLanguage != item.code" as="button" @click="setDefaultAction(item.code)">{{ $t('Set as default') }}</DropdownItem>
