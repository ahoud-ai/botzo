            ...
            <TableBody>
                <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                    ...
                    <TableBodyRowItem class="hidden sm:table-cell">
                        <div class="py-1 px-2 bg-gray-50 rounded-md border border-dashed w-[10em] truncate text-xs capitalize">
                            {{ item.trigger }}
                        </div>
                    </TableBodyRowItem>
                    ...
                    <TableBodyRowItem class="capitalize">
                        <span class="py-1 rounded-md text-xs px-3 ui-chip-neutral text-slate-700">{{ $t(JSON.parse(item.metadata).type) }}</span>
                    </TableBodyRowItem>
