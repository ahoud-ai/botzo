<script setup>
    ...
    const deleteAction = (key) => {
        ...
        form.delete('/automation/upload/document/' + key);
    }
...
<template>
    <div class="ui-table-search">
        ...
        <input @input="search" v-model="params.search" type="text" class="ui-table-search-input" :placeholder="$t('Search by title')">
    ...
    <div class="bg-[var(--ui-surface-soft)] rounded-[0.5rem]">
        <table class="w-full">
            <TableHeader>
                <TableHeaderRow>
                    <TableHeaderRowItem :position="'first'">{{ $t('Title') }}</TableHeaderRowItem>
                    <TableHeaderRowItem>{{ $t('Source') }}</TableHeaderRowItem>
            ...
            <TableBody>
                <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                    <TableBodyRowItem :position="'first'" class="capitalize">{{ item.title }}</TableBodyRowItem>
                    <TableBodyRowItem class="hidden sm:table-cell">
                        {{ item.source }}
                    </TableBodyRowItem>
                    <TableBodyRowItem class="hidden sm:table-cell">{{ $t(capitalizeString(item.status)) }}</TableBodyRowItem>
                    <TableBodyRowItem class="hidden sm:table-cell">{{ item.updated_at }}</TableBodyRowItem>
                    <TableBodyRowItem :position="'last'">
                        <Dropdown v-if="canDelete" :align="'right'" class="mt-2">
                            <button type="button" @click="openAlert(item.uuid)" class="inline-flex w-full justify-center rounded-md text-sm font-medium text-black hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
