<script setup>
    ...
    const { t, te } = useI18n();
    const { canManageTeam } = useWorkspaceAccess();
    const { isOpenAlert, openAlert, confirmAlert } = useAlertModal();

    const form = useForm({'test': null});

    const deleteAction = (key) => {
        form.delete('/settings/team/' + key);
    }
    ...
    function edit(id, organization_role_id, email) {
        emit('edit', { id: id, organization_role_id: organization_role_id, email: email });
    }
    ...
    function displayRole(item) {
        return localizeKnownValue(item.role_name || item.organization_role?.name);
    }
</script>
<template>
    ...
    <Table :rows="rows">
        ...
        <TableBody>
            <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                ...
                <TableBodyRowItem class="hidden sm:table-cell">{{ displayRole(item) }}</TableBodyRowItem>
                <TableBodyRowItem class="capitalize hidden sm:table-cell">
                    <span class="py-1 rounded-md text-xs px-3 ui-chip-neutral text-slate-700">{{ localizeKnownValue(item.status) }}</span>
                </TableBodyRowItem>
                ...
                <TableBodyRowItem v-if="canManageTeam" :position="'last'">
                    <Dropdown v-if="item.organization_role && item.organization_role.organization_id" :align="'right'" class="mt-2">
                        ...
                        <template #items>
                            <DropdownItemGroup>
                                <DropdownItem as="button" @click="edit(item.uuid, item.organization_role_id || item.role_id, item.user.email)">{{ $t('Edit') }}</DropdownItem>
