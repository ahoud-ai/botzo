        ...
        <TableBody>
            <TableBodyRow v-for="(item, index) in rows.data" :key="index">
                ...
                <TableBodyRowItem class="hidden sm:table-cell">
                    <Link :href="ticketUrl(item.uuid)">
                        <div class="py-1 px-2 bg-gray-50 rounded-md border border-dashed w-[20em] truncate text-xs capitalize">
                            {{ item.subject }}
                        </div>
                    </Link>
                </TableBodyRowItem>
                <TableBodyRowItem class="hidden sm:table-cell">
                    <Link :href="ticketUrl(item.uuid)">
                        <span class="text-start bg-slate-100 py-2 px-3 rounded-xl text-[11px]">{{ localizeKnownValue(item.priority, $t('Not set')) }}</span>
                    </Link>
                </TableBodyRowItem>
