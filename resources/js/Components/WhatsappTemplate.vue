...
<template>
    <div v-if="visible" class="me-auto rounded-lg rounded-tl-none my-[0.1em] p-1 text-sm bg-white dark:bg-[#202c33] flex flex-col relative speech-bubble-left max-w-[25em]">
        <div v-if="parameters.header.format != null && parameters.header.format != 'TEXT'">
            <div v-if="placeholder == true" class="ui-bg-soft mb-4 flex justify-center rounded py-8">
                <img v-if="parameters.header.format === 'IMAGE'" :src="'/images/image-placeholder.png'" alt="">
                <img v-if="parameters.header.format === 'VIDEO'" :src="'/images/video-placeholder.png'" alt="">
                <img v-if="parameters.header.format === 'DOCUMENT'" :src="'/images/document-placeholder.png'" alt="">
            </div>
            <div v-else class="mb-4 flex justify-center rounded">
                <div v-if="parameters.header.format === 'IMAGE'" class="relative w-full h-full">
                    <img :src="parameters.header.parameters[0].value" :alt="$t('Image')" class="rounded-md flex justify-center items-center z-1 w-full h-full" />
                </div>
        ...
        <h2 v-else class="text-gray-700 dark:text-slate-100 text-[16px] mb-1 px-2">{{ parameters.header.text }}</h2>
    ...
    <div v-if="parameters?.buttons?.length > 0" class="me-auto text-sm ui-text-primary flex flex-col relative max-w-[25em]">
        <div v-for="(item, index) in parameters.buttons" :key="index" class="flex justify-center items-center gap-x-2 rounded-lg bg-white dark:bg-[#202c33] h-10 my-[0.1em]">
            <span>
