...
<template>
    <div class="relative inline-block text-sm">
        ...
        <div v-if="isOpen" class="ui-dropdown-menu ui-layer-dropdown ui-dropdown-end absolute mt-2 w-[7em] p-1 rounded-md text-[var(--ui-text)]">
            <div>
                <div @click="sort('desc')" class="ui-dropdown-item block px-2 py-1 cursor-pointer hover:bg-slate-100 rounded-md">
                    {{ $t('Newest') }}
                </div>
                <div @click="sort('asc')" class="ui-dropdown-item block px-2 py-1 cursor-pointer hover:bg-slate-100 rounded-md">
                    {{ $t('Oldest') }}
                </div>
            </div>
        </div>
