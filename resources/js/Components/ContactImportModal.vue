...
<template>
    ...
    <Modal :label="modalLabel" :isOpen="isOpenModal" @close="isOpenModal = false">
        <div v-if="type === 'contact'" class="text-sm text-slate-600 mt-4">{{ $t("Upload a csv/xlsx to import your contact data. For the phone field ensure that you start with the contact's country code.") }}</div>
            ...
            <div class="space-y-6">
                ...
                <div v-else class="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                    <div>
                        ...
                        <div class="text-center mb-2 text-sm text-gray-500">{{ $t('Upload In Progress') }}</div>
                        ...
                        <div v-if="flashResponse.import_summary.failed_rows_details && flashResponse.import_summary.failed_rows_details.length > 0">
                            <div class="mt-2">
                                <div class="bg-slate-50 text-sm grid grid-cols-2 p-1 py-2 rounded-md border-b-2 border-white">
                                    <div class="col-1 ps-2">{{ $t('Row') }}</div>
                                    <div class="col-1 flex justify-between items-center">
                                        <span>{{ $t('Error') }}</span>
                                        <button type="button" @click="toggleFailedTable" class="bg-primary text-white text-[11px] rounded px-2">{{ showFailedTable ? $t('Hide') : $t('Show') }}</button>
                                    </div>
                                </div>
                                <div v-if="showFailedTable" class="bg-red-50 text-[12px] grid grid-cols-2 border-white border-x border-b p-1 rounded-md" v-for="(item, index) in flashResponse.import_summary.failed_rows_details" :key="index">
                                    <div class="col-1 ps-2">{{ item.row }}</div>
