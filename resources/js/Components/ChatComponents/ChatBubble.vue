<script setup>
    ...
    import { interactiveBodyText, parseChatMetadata } from './chatMessagePreview.js';
    ...
    const { t } = useI18n();
    const metadata = computed(() => parseChatMetadata(props.content?.metadata));
    const interactive = computed(() => metadata.value?.interactive || {});
    const interactiveType = computed(() => interactive.value?.type || '');
    const interactiveHeaderText = computed(() => interactive.value?.header?.text || '');
    const interactiveFooterText = computed(() => interactive.value?.footer?.text || '');
    const interactiveMessageBody = computed(() => interactiveBodyText(metadata.value));
    const interactiveButtons = computed(() => Array.isArray(interactive.value?.action?.buttons) ? interactive.value.action.buttons : []);
    const interactiveSections = computed(() => Array.isArray(interactive.value?.action?.sections) ? interactive.value.action.sections : []);
    const interactiveListButton = computed(() => interactive.value?.action?.button || '');
    const interactiveCtaParameters = computed(() => interactive.value?.action?.parameters || {});
    const interactiveCtaText = computed(() => interactiveCtaParameters.value?.display_text || interactiveCtaParameters.value?.url || '');
    const inboundInteractiveTitle = computed(() => {
        if (interactiveType.value === 'button_reply') {
            return interactive.value?.button_reply?.title || interactive.value?.button_reply?.id || '';
        }

        if (interactiveType.value === 'list_reply') {
            return interactive.value?.list_reply?.title || interactive.value?.list_reply?.id || '';
        }

        return '';
    });
    const inboundInteractiveDescription = computed(() => interactiveType.value === 'list_reply' ? interactive.value?.list_reply?.description || '' : '');
    ...
    const chatStatus = (logs) => {
        // First check if the chat itself has a failed status (from chats.status column)
        // This is the primary check since failed status is stored in the chats.status column
        if (props.content?.status === 'failed') {
            return 'failed';
        }

        let status = 'sent';

        // If no logs, return the chat's status or default to 'sent'
        if (!logs || logs.length === 0) {
            // For failed messages, status should be 'failed' from the chats table
            return props.content?.status === 'failed' ? 'failed' : (props.content?.status || 'sent');
        }
        ...
        logs.forEach(log => {
            try {
                ...
                // If metadata parsing fails, continue to next log
                console.error('Error parsing log metadata:', e);
            }
        });

        // Final check: if chat status is failed, override any other status
        if (props.content?.status === 'failed') {
            return 'failed';
        }
    ...
    const errorDetails = computed(() => {
        try {
            // Check if chat status is failed and look for error in ChatStatusLog
            if (props.content?.status === 'failed') {
                // Look for ChatStatusLog with error metadata
                if (props.content?.logs && props.content.logs.length > 0) {
                    for (let log of props.content.logs) {
                        try {
                            const metadata = JSON.parse(log.metadata);
                            // Check if this is an error log (has error field or status is failed)
                            if (metadata.status === 'failed' || metadata.error) {
                                return {
                                    message: metadata.message || metadata.error?.message || 'Unknown error',
                                    error_code: metadata.error_code || metadata.error?.code || null,
                                    error_type: metadata.error_type || metadata.error?.type || null,
                                    error_user_msg: metadata.error_user_msg || metadata.error?.error_user_msg || null,
                                    error_data: metadata.error?.error_data || null,
                                    fbtrace_id: metadata.fbtrace_id || metadata.error?.fbtrace_id || null,
                                    full_error: metadata.error || null
                                };
                            }
                        } catch (e) {
                            continue;
    ...
    const copyFullError = async () => {
        try {
            // Find the ChatStatusLog entry with error metadata
            if (props.content?.logs && props.content.logs.length > 0) {
                for (let log of props.content.logs) {
                    try {
                        const metadata = JSON.parse(log.metadata);
                        if (metadata.status === 'failed' || metadata.error) {
                            // Format the full error metadata as JSON
                            const fullError = JSON.stringify(metadata, null, 2);
                            copiedRef.value = 'error';
                            
                            const tempInput = document.createElement("textarea");
                            tempInput.value = fullError;
                            document.body.appendChild(tempInput);
                            tempInput.select();
                            document.execCommand("copy");
                            document.body.removeChild(tempInput);
            ...
            console.error("Error copying full error:", error);
        }
    };
</script>
<template>
        ...
        <div>
            ...
            <div v-else-if="JSON.parse(content.metadata).type === 'document'">
                <div class="relative w-[300px]">
                    <div class="flex gap-x-2 w-full h-1/3 bg-white opacity-90 pt-2">
                        <div>
            ...
            <div v-else-if="JSON.parse(content.metadata).type === 'audio'">
                <audio v-if="content.media?.path" controls>
