    ...
    <div class="py-4 md:py-4 relative px-6 md:px-10">
        <div v-if="hasMore" 
            ...
            <div v-if="loading" 
                ...
                <svg class="animate-spin -ms-1 me-3 h-5 w-5 text-gray-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div v-if="chat[0].type === 'notes'" class="py-2 bg-orange-100 my-2 rounded-lg p-2 w-[fit-content] ms-auto">
                <div class="text-end font-light text-sm">
                    ...
                    <div class="flex items-center justify-between mt-2 gap-x-4">
                        <!--<span v-if="props.type === 'outbound' && content.user" class="text-gray-500 text-xs text-end leading-none">Sent By: {{ content.user?.first_name + ' ' + content.user?.last_name }}</span>-->
                        <p class="text-gray-500 text-xs text-end leading-none">{{ chat[0].value.created_at }}</p>
                    </div>
                </div>
            </div>
        </div>
