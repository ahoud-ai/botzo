...
<template>
    <div class="flex h-full min-h-0 w-full overflow-y-auto">
        <div class="pb-6 border-b">
            <div class="flex justify-center w-full">
                <div class="rounded-full p-1">
                    <img v-if="contact.avatar" class="rounded-full w-20 h-20" :src="contact.avatar" alt="">
                    <div v-else class="rounded-full w-20 h-20">
                        <svg class="text-gray-300 dark:text-slate-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            ...
            <div>
                ...
                <div class="text-slate-500 dark:text-slate-400 truncate flex items-center justify-center">
                    <span class="text-sm">{{ contact.formatted_phone_number }}</span>
                </div>
                <div class="flex gap-x-3 mt-4 justify-center">
                    <button v-if="canEdit" type="submit" class="bg-gray-200 dark:bg-white/10 px-4 rounded-md flex items-center">
                        <Link :href="'/contacts/' + contact.uuid + '?edit=true'" class="text-sm">{{ $t('Edit') }}</Link>
                    </button>
                    <button type="button" @click="favorite()" class="bg-gray-200 dark:bg-white/10 px-4 rounded-md">
                    ...
                    <Dropdown v-if="canDelete" class="h-100" :align="'right'">
                        <button type="submit" class=" bg-gray-200 dark:bg-white/10 py-2 px-3 rounded-md flex items-center">
                            <span class="">
        ...
        <div class="py-4 px-4 space-y-1 text-sm">
            <h4 class="mb-4">{{ $t('Contact Details') }}</h4>
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('Group') }}</span>
                </div>
                <div>
                    <span class="p-1 bg-gray-50 dark:bg-white/5 text-xs rounded-md text-gray-500 dark:text-slate-400 capitalize px-2">{{ contact.contact_group?.name ?? $t('not set') }}</span>
                </div>
            </div>
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('Email') }}</span>
                </div>
                <div>
        ...
        <div v-if="locationSettings === 'before' && fields.length > 0" class="border-gray-300 dark:border-white/10 border-b py-4">
            <div v-for="(input, index) in props.fields" class="grid grid-cols-2 gap-x-2 text-sm">
                <div class="text-end text-slate-500 dark:text-slate-400 pb-2">
                    <span>{{ $t(input.name) }}</span>
                </div>
                <div>
                    <span v-if="metadata[input.name] != null">{{ metadata[input.name] }}</span>
                    <span v-else class="p-1 bg-slate-100 dark:bg-white/10 text-xs rounded-md text-gray-600 dark:text-slate-400">{{ $t('not set') }}</span>
                </div>
            </div>
        </div>
        <div class="border-t py-4 px-4 space-y-1 text-sm">
            <h4 class="mb-4">{{ $t('Address') }}</h4>
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('Street') }}</span>
                </div>
                <div>
            ...
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('City') }}</span>
                </div>
                <div>
            ...
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('State') }}</span>
                </div>
                <div>
            ...
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('Zip code') }}</span>
                </div>
                <div>
            ...
            <div class="flex">
                <div class="w-[30%]">
                    <span class="text-slate-500 dark:text-slate-400">{{ $t('Country') }}</span>
                </div>
                <div>
        ...
        <div class="border-t py-4 px-4 space-y-1">
            <div class="flex">
                <div class="mb-3">
                    <h4 class="mb-0">{{ $t('Notes') }}</h4>
                    <span class="text-xs text-slate-500 dark:text-slate-400">{{ $t('Store notes for your team') }}</span>
                </div>
                <div class="ms-auto">
                    <button type="button" @click="isOpenModal = true;" class="bg-slate-50 dark:bg-white/5 p-2 rounded-full shadow-md hover:bg-slate-100 dark:hover:bg-white/10 hover:shadow-xl">
            ...
            <div v-for="(item, index) in contact.notes" :key="index" class="bg-gray-50 dark:bg-white/5 text-sm p-2 rounded-md">
                <div class="mb-1 font-regular">{{ item.content }}</div>
                <span class="font-light text-gray-500 dark:text-slate-400 text-xs">{{ item.created_at }}</span>
            </div>
        </div>
        <div v-if="locationSettings === 'after' && fields.length > 0" class="pe-20 border-gray-300 dark:border-white/10 border-t py-4">
            <div v-for="(input, index) in props.fields" class="grid grid-cols-2 gap-x-8 text-[14px]">
                <div class="text-end text-slate-500 dark:text-slate-400 pb-2">
                    <span>{{ $t(input.name) }}</span>
                </div>
                <div>
