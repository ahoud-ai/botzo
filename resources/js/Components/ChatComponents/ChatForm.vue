<script setup>
    ...
    const assertSuccessfulSend = (response) => {
    ...
    const sendMessage = async() => {
        ...
        const hasPrimaryMessage = (form.value.message != null && form.value.message.trim() !== '') || form.value.file != null;
        const hasAudioMessage = isAudioRecording.value === true && form2.value.file != null;

        if (!hasPrimaryMessage && !hasAudioMessage) {
            return;
        }
        ...
        try {
            ...
            sendError.value = error.response?.data?.message || error.message || t('Unable to send message');
        } finally {
    ...
    const replyWindowOpen = computed(() => isInboundChatWithin24Hours.value);
    const replyWindowTitle = computed(() => '24 hour limit');
    const replyWindowDescription = computed(() => 'Whatsapp does not allow sending messages 24 hours after they last messaged you. However, you can send them a template message');

    const appendReplyContext = (formData) => {
        formData.append('channel', activeReplyChannel.value);
    };
    ...
    const handlePlaybackEnded = () => {
        isPlaying.value = false;
        stopPlaybackTimer();
        playbackTime.value = audioDuration.value || playbackTime.value;
    };
    ...
    const handleAudioLoaded = () => {
        if (audioPlayer.value && Number.isFinite(audioPlayer.value.duration)) {
    ...
    const getSuggestion = async() => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                sendError.value = response.data.message || 'Unable to generate a suggestion';
            }
        } catch (error) {
            sendError.value = error.response?.data?.message || error.message || 'Unable to generate a suggestion';
        } finally {
...
<template>
    <div v-if="!canReply" class="flex justify-center items-center w-full px-6 md:px-4">
        <div class="flex items-start gap-x-4 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-3 text-sm text-slate-600 dark:text-slate-400">
            <span>{{ $t('You can view this conversation, but you do not have permission to send replies.') }}</span>
        </div>
    </div>
    <div v-if="canReply && props.chatLimitReached" class="flex justify-center items-center w-full px-6 md:px-4">
        <div class="flex items-start gap-x-4 bg-orange-100 dark:bg-orange-500/10 rounded-lg p-2 mb-2 px-4">
            <span class="text-red-700 dark:text-orange-400">
    ...
    <div v-if="canReply && !replyWindowOpen && !props.chatLimitReached" class="flex justify-center items-center w-full px-6 md:px-4">
        <div class="flex items-center justify-between gap-x-4 bg-orange-100 dark:bg-orange-500/10 rounded-lg p-2 mb-2 px-4">
            <div class="flex items-start justify-between gap-x-4">
                <span class="text-red-700 dark:text-orange-400">
                    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 36 36"><path fill="currentColor" d="M18 21.32a1.3 1.3 0 0 0 1.3-1.3V14a1.3 1.3 0 1 0-2.6 0v6a1.3 1.3 0 0 0 1.3 1.32Z" class="clr-i-outline clr-i-outline-path-1"/><circle cx="17.95" cy="24.27" r="1.5" fill="currentColor" class="clr-i-outline clr-i-outline-path-2"/><path fill="currentColor" d="M30.33 25.54L20.59 7.6a3 3 0 0 0-5.27 0L5.57 25.54A3 3 0 0 0 8.21 30h19.48a3 3 0 0 0 2.64-4.43Zm-1.78 1.94a1 1 0 0 1-.86.49H8.21a1 1 0 0 1-.88-1.48l9.74-17.94a1 1 0 0 1 1.76 0l9.74 17.94a1 1 0 0 1-.02.99Z" class="clr-i-outline clr-i-outline-path-3"/><path fill="none" d="M0 0h36v36H0z"/></svg>
                </span>
                <div>
                    <div class="text-sm">{{ $t(replyWindowTitle) }}</div>
                    <div class="text-sm">{{ $t(replyWindowDescription) }}</div>
                </div>
            </div>
            <button type="button" @click="viewTemplate()" class="rounded-md bg-primary px-3 py-1 text-sm text-white shadow-sm w-[25%]">{{ $t('Send Template') }}</button>
        </div>
    </div>
    <form v-if="canReply && replyWindowOpen && !props.chatLimitReached" @submit.prevent="sendMessage()" class="flex items-center px-2 md:px-10 gap-x-2">
        <div class="bg-white dark:bg-[#111b21] border border-slate-200 dark:border-white/10 rounded-xl w-full p-4 shadow-sm">
            <div v-if="sendError" class="mb-2 text-sm text-red-600">
                {{ sendError }}
            </div>
            <div>
                <textarea 
                    ...
                    class="w-full outline-none resize-none text-[14.5px] rounded-md p-2"
                    :class="processingForm ? 'bg-gray-100 dark:bg-white/10' : 'bg-white dark:bg-[#111b21]'"
            ...
            <!-- Recording Interface -->
            <div v-if="isRecording || audioPreviewUrl" class="bg-slate-50 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 px-2 py-1 mb-2">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm" v-else>{{ formatTime(playbackTime) }} / {{ formatTime(audioDuration || recordingTime) }}</span>
                    </div>
                    <div class="flex gap-2">
                        ...
                        <button type="button" 
                            v-if="isRecording"
                            @click="stopRecording"
                            class="p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-500/10"
                        ...
                        <button type="button" 
                            ...
                            class="p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-500/10 text-red-500"
                ...
                <audio ref="audioPlayer" :src="audioPreviewUrl" @loadedmetadata="handleAudioLoaded" @ended="handlePlaybackEnded" class="hidden" />
            </div>
            <div class="flex gap-x-1 items-center text-gray-600 dark:text-slate-400 flex-wrap">
                <button type="submit" class="chat-toolbar-btn chat-toolbar-btn--wide" @click="viewTemplate()">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256"><path fill="currentColor" d="M216 80h-32V48a16 16 0 0 0-16-16H40a16 16 0 0 0-16 16v128a8 8 0 0 0 13 6.22L72 154v30a16 16 0 0 0 16 16h93.59L219 230.22a8 8 0 0 0 5 1.78a8 8 0 0 0 8-8V96a16 16 0 0 0-16-16M66.55 137.78L40 159.25V48h128v88H71.58a8 8 0 0 0-5.03 1.78M216 207.25l-26.55-21.47a8 8 0 0 0-5-1.78H88v-32h80a16 16 0 0 0 16-16V96h32Z"></path></svg>
                    <span class="text-sm">{{ $t('Templates') }}</span>
                </button>
                <span class="chat-toolbar-divider"></span>
                <div class="flex items-center gap-x-1">
                    <div class="relative">
                        <button type="button" class="chat-toolbar-btn" @click="toggleEmojiPicker">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.111 2.18a7 7 0 1 1 7.778 11.64A7 7 0 0 1 4.11 2.18zm.556 10.809a6 6 0 1 0 6.666-9.978a6 6 0 0 0-6.666 9.978M6.5 7a1 1 0 1 1-2 0a1 1 0 0 1 2 0m5 0a1 1 0 1 1-2 0a1 1 0 0 1 2 0M8 11a3 3 0 0 1-2.65-1.58l-.87.48a4 4 0 0 0 7.12-.16l-.9-.43A3 3 0 0 1 8 11" clip-rule="evenodd"/></svg>
                        </button>
                        <div v-if="emojiPicker" class="absolute left-0 bottom-full z-10" ref="emojiPickerRef">
                            <EmojiPicker :native="true" @select="addEmoji" />
                        </div>
                    </div>
                    <label class="chat-toolbar-btn" @click="form.type = 'document'" for="file-upload">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 56 56"><path fill="currentColor" d="M44.09 28.867L26.863 46.094c-4.476 4.5-10.5 4.054-14.343.164c-3.867-3.844-4.313-9.82.187-14.32L36.191 8.453c2.696-2.695 6.657-3.07 9.235-.515c2.555 2.601 2.18 6.539-.492 9.234L21.988 40.14c-1.171 1.195-2.554.843-3.375.047c-.796-.82-1.125-2.18.047-3.376l16.031-16.007c.704-.727.75-1.758.07-2.438c-.679-.656-1.71-.61-2.413.094L16.246 34.563c-2.39 2.39-2.297 6.046-.187 8.156c2.297 2.297 5.789 2.25 8.18-.164l23.085-23.063c4.383-4.383 4.172-10.148.375-13.969c-3.75-3.726-9.586-4.007-13.992.375L10.082 29.547c-5.789 5.789-5.344 14.062-.117 19.289c5.227 5.203 13.5 5.648 19.289-.117l17.344-17.344c.703-.68.68-1.922-.024-2.555c-.68-.726-1.78-.633-2.484.047"/></svg>
                    </label>
                    <label class="chat-toolbar-btn" @click="form.type = 'audio'" for="file-upload">
                    ...
                    <label class="chat-toolbar-btn" @click="form.type = 'video'" for="file-upload">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 32 32"><path fill="currentColor" d="M6.5 5.5A4.5 4.5 0 0 0 2 10v12a4.5 4.5 0 0 0 4.5 4.5h12A4.5 4.5 0 0 0 23 22v-1.5l4.2 3.15c1.153.865 2.8.042 2.8-1.4V9.75c0-1.442-1.647-2.265-2.8-1.4L23 11.5V10a4.5 4.5 0 0 0-4.5-4.5zM23 14l5-3.75v11.5L23 18zm-2-4v12a2.5 2.5 0 0 1-2.5 2.5h-12A2.5 2.5 0 0 1 4 22V10a2.5 2.5 0 0 1 2.5-2.5h12A2.5 2.5 0 0 1 21 10"/></svg>
                    </label>
                    <label class="chat-toolbar-btn" @click="form.type = 'image'" for="file-upload">
                ...
                <div v-if="props.showAiAssist">
                    <button v-if="!processingAISuggestions" class="chat-toolbar-btn chat-toolbar-btn--wide" type="button" :disabled="processingForm" @click="getSuggestion()">
                        ...
                        <span class="text-sm">{{ $t('AI Assist') }}</span>
                    </button>
                    <div v-else class="chat-toolbar-btn chat-toolbar-btn--wide chat-toolbar-btn--active">
                        <span class="text-sm">{{ $t('Searching for suggestions') }}</span>
                ...
                <div class="ms-auto">
                    <button type="submit" class="chat-send-btn" :class="{ 'chat-send-btn--active': !((formTextInput === null || formTextInput.trim() === '') && !form2.file) }" :disabled="processingForm || ((formTextInput === null || formTextInput.trim() === '') && !form2.file)">
                        <span class="text-sm font-medium">{{ $t('Send') }}</span>
                        <svg v-if="!processingForm" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 16 16"><path fill="currentColor" d="M1.724 1.053a.5.5 0 0 0-.714.545l1.403 4.85a.5.5 0 0 0 .397.354l5.69.953c.268.053.268.437 0 .49l-5.69.953a.5.5 0 0 0-.397.354l-1.403 4.85a.5.5 0 0 0 .714.545l13-6.5a.5.5 0 0 0 0-.894l-13-6.5Z"/></svg>
                        <svg v-else xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24"><circle cx="12" cy="2" r="0" fill="currentColor"><animate attributeName="r" begin="0" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(45 12 12)"><animate attributeName="r" begin="0.125s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(90 12 12)"><animate attributeName="r" begin="0.25s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(135 12 12)"><animate attributeName="r" begin="0.375s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(180 12 12)"><animate attributeName="r" begin="0.5s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(225 12 12)"><animate attributeName="r" begin="0.625s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(270 12 12)"><animate attributeName="r" begin="0.75s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(315 12 12)"><animate attributeName="r" begin="0.875s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle></svg>
                    </button>
                </div>
            </div>
        </div>
