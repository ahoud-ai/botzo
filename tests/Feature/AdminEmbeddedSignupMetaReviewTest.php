...
class AdminEmbeddedSignupMetaReviewTest extends TestCase
{
    use DatabaseTransactions;

    public function test_admin_can_run_embedded_signup_meta_review_tests(): void
    {
        ...
        $response = $this->actingAs($admin, 'admin')
            ->postJson('/admin/settings/features/embedded-signup/meta-review-tests');

        $response->assertOk()
            ->assertJsonPath('status', 'passed')
            ->assertJsonPath('summary.passed', 3)
            ->assertJsonPath('tests.0.key', 'business_management')
            ->assertJsonPath('tests.1.key', 'whatsapp_business_management')
            ->assertJsonPath('tests.2.key', 'whatsapp_business_messaging');
    }

    public function test_meta_review_tests_report_failure_when_access_token_is_missing(): void
    {
        $admin = $this->createAdminUser();
        $this->seedEmbeddedSignupSettings([
    ...
    public function test_admin_meta_review_tests_fall_back_to_known_waba_owner_business_info_when_business_list_is_empty(): void
    {
        $admin = $this->createAdminUser();
        $this->seedEmbeddedSignupSettings();
        ...
        $response->assertOk()
            ->assertJsonPath('status', 'passed')
            ->assertJsonPath('summary.passed', 3)
            ->assertJsonPath('tests.0.status', 'passed')
            ->assertJsonPath('tests.0.meta.resolution_strategy', 'waba_owner_business_info');
    }
