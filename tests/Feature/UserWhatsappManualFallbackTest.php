...
class UserWhatsappManualFallbackTest extends TestCase
{
    use DatabaseTransactions;

    public function test_whatsapp_settings_exposes_global_disabled_manual_fallback(): void
    {
        ...
        $this->seedPlanEnabledSubscription($organization->id);
        ...
        $response->assertInertia(fn (Assert $page) => $page
            ->where('manualFallbackActive', true)
            ->where('manualFallbackReason', 'global_disabled')
            ->where('embeddedSignupActive', false)
        );
    }

    public function test_whatsapp_settings_exposes_org_disabled_manual_fallback(): void
    {
        [$user, $organization] = $this->createUserAndOrganization([
            'addons' => [
        ...
        $response->assertInertia(fn (Assert $page) => $page
            ->where('manualFallbackActive', true)
            ->where('manualFallbackReason', 'org_disabled')
            ->where('embeddedSignupActive', false)
        );
    }

    public function test_whatsapp_settings_exposes_plan_disabled_manual_fallback(): void
    {
        [$user, $organization] = $this->createUserAndOrganization([
            'addons' => [
                'embedded_signup_enabled' => true,
        ...
        $this->seedEmbeddedSignupAddonAndGlobalFlag(true);
        $this->seedPlanDisabledSubscription($organization->id);
        ...
        $response->assertInertia(fn (Assert $page) => $page
            ->where('manualFallbackActive', true)
            ->where('manualFallbackReason', 'plan_disabled')
            ->where('embeddedSignupActive', false)
        );
    }
    ...
    private function seedPlanEnabledSubscription(int $organizationId): void
    {
    ...
    private function seedPlanDisabledSubscription(int $organizationId): void
    {
        $plan = SubscriptionPlan::create([
            'name' => 'Starter '.Str::random(5),
            'price' => 10,
